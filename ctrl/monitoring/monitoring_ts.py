...
for configSection in configSections:
    try:
        ...
        tmpDataDir = f'{tmpDataDir}/{varName}_ts.nc'
        ...
        if SanityKind == "mean":
            data = np.ma.mean(data, axis=(-2,-1))
            data = np.ma.mean(data, keepdims=True)
        if SanityKind == "sum":
            data = np.ma.sum(data, axis=(-2,-1))
            data = np.ma.sum(data, keepdims=True)
        ...
        if os.path.isfile(tmpDataDir):
            with nc.Dataset(tmpDataDir, 'a') as nc_file:
                nc_data  = nc_file.variables[varName]
                nc_time  = nc_file.variables['time'] 
                idx_max  = nc_data.shape[0]

                nc_data[idx_max] = data
                data = nc_data[...]
                ...
                time = tmp_time
                tmp_data = data[timesortidx]
                data = tmp_data
        else:
            with nc.Dataset(tmpDataDir, 'w', format='NETCDF4') as nc_file:
        ...
        shortMeanPeriod = 12
        data_alltime_mean = np.mean(data, keepdims=True)
        ...
        if data.shape[0] >= shortMeanPeriod:
            data_shorttime_mean = np.mean(data[-1*shortMeanPeriod:])
        ...
        xvalues = (time - np.min(time))/24. #np.arange(data.shape[0])
        ax.plot(xvalues, data - data_alltime_mean, label='alltime anomaly') 
        ax.plot(xvalues[-1*shortMeanPeriod:], data[-1*shortMeanPeriod:] - data_shorttime_mean, label=f'shorttime anomaly (shortMeanPeriod: {shortMeanPeriod})') 
